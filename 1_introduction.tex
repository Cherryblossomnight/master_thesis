...
The aerial manipulation tasks are very important. One of the most challenge task is the surface sliding tasks.

All of these tasks have the following characteracstics: The task need the robot to realize (a) The end effector of aerial robot need to keep long time contact with the surface. (b) The end effector of the aerial robot need to generate related tangential movement related to the surface. The real enviroments bring the following difficulties to the tasks: (c) The contact force (including tangnetial and normal force) between the end effector and the surface may influence the performance of the aerial robot. (d) The surface geometry may be compex in the real world, the friction factor and the geometry is usually changing and unmodelled for the controller. Considering the above features, the surface sliding tasks need to . 

In the current literatures, the aerial robots are require to solve the manipulation tasks like bridge inspection \cite{bodie_2020_active}, infrastructure inspection \cite{guo_2024_flying}, tube inspection \cite{trujillo_2019_novel} and writing \cite{guo_2024_flying}, and. Apparently, the requirements in reality of surface sliding tasks are very high.

Currently, although there are a lot of researches pay attention to surface sliding tasks, they (a) do not collect and summaize the main features of this kind of manipulation tasks (b) did not express the effectice framework to solve the proposed problems. 


\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/chapter1/surface_sliding.png}
  \caption{Example scenario of surface sliding task, mainly derived from infrasturcture maintainese. (a) The aerial robot sliding on the bridge's curve surface to inspect the cracks. (b) The aerial robot sliding on the steel tube to inspect the weld seam. (c) (d).}
  \label{fig:surface sliding}
  \vspace{-7mm}
\end{figure}
...
\begin{figure}[t]
  ...
  \caption{Four morphologies of aerial robots. (a) Single rigid body morphology. (b) Sseries morphology. (c) Parallel morphology. (d) multi-link morphology}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/chapter1/morphologies.png}
  \caption{Some example tasks that use impedance or admittance control. (a) The impedance controller is used as a adjustment. (b) The impedance controller's parameters are tuning by the learning-based method, which helps the aerial robot achieve sliding on the uneven surface. (c) The admittance controller as a filter to adapt to the external disturbance. (d)}
  \label{fig:force control}
  \vspace{-7mm}
\end{figure}
...
\begin{itemize}
	\item Chapter 1 introduces the research background, motivation and main contributions of this study, reviews related work about the defination and solution of surface sliding tasks, morphologies of aerial robots and force control methods used in aerial manipulation. Based on above discussion, the motivation, objectives and contributions of this thesis are introduced.
	\item Chapter 2 presents the preliminary of this thesis, including the notation defination, the modeling of multi-link aerial robots and the external wrench estimated algorithm.
	\item Chapter 3 introduces the basic concepts of impedance control, analyzes how the impedance control can redesign the dynamic parameters of a system, and a new insight of the impedance parameters. The Implementation of the multi-link aerial robots and the task-oritation coordinate transformation method are also introduced. 
	\item Chapter 4 introduces the basic concepts of admittance control. The admittance control is used for adjust the internal configuration of the aerial robot, while perform adaptation to the external geometric variations.
	\item Chapter 5 introduces how to combined impedance control and admittance control together as the hybrid impedance--admittance control on the same system. The effects of the intergrated hybrid controller is showed on the multi-link aerial robots.
	\item Chapter 6 presents experimental results. The parameter is discussed. The Hydrus is selected to demonstrated the main concept, and DRAGON is used for showing the adaptability of the proposed framework in multi-link aerial robots.
