...
The morphology is very important for the aerial manipulation. As we all know.
The morphologies of aerial manipulation can be briefly divided into four types: single rigid body manipulator, parrel manipulator, series manipulator and other specific type like multi-link aerial robot. The structure is important for aerial manipulation\cite{ollero_2021_past} as illustrated in Fig.~\ref{fig:morphologies}.

The single rigid body manipulator is just install a rod or a stick on the aerial robot, the manipulator usually keeps static to the aerial robot. Because this kind of morphology is very simple, we don’t need to think about how to control the end effector itself. Therefore, the control method for the single rigid body manipulator has developed a lot. Generally, the rigid manipulator is used for task like push-and-slide and peg-in-hole. The end effector is usually a pad that installed with sensors, or a roller to move in the surface smoothly. The main requirement is the pose of manipulator, and it is ragulate by aerial robot's pose. For more complicated tasks, the rigid manipulator may be not enough. Meanwhile， the rigid manipulator do not have extra d.o.f, so if the requirements need more, this type is not good. In the recent research. For example Bodie et al. \cite{bodie_2020_active} uses a static end effector to inspect bridge. Brunner et al. \cite{brunner_2022_planning} realized to operate an articulated door. Actually, they did not use the manipulator it self to do the tasks.

The parallel manipulator, also the delta manipulator, is usually used in high precision and high speed position control in a ground robot. However, the work space of this kind of morphology is too small, therefore is hard to be used in quick tasks. Guo et al. \cite{guo_2024_flying} uses delta arm to draw on a board. Deng et al. \cite{deng_2025_whole} realized using delta arm to push objects, which is very fast. We can see Deng's work show that the delta manipulator is very good at high speed tasks. However, the work space of delta manipulator is too small, and it also would lead too much weight.

The serial manipulator, also the robotic arm manipulator, is usually used in large working space and obstacle avoidance scenario. However, the serial manipulator. Too support the hole arm, it needs high quality rotor, which will also bring high weight.
He et al. \cite{he_2025_flying} used serial arm to draw on a board. Lee et al. \cite{lee_2025_autonomous} realized SE(3) operation like peg-in-hole.

Finally the multi-link aerial robot. This type of aerial robot can be regarded as a flying robotic arm, so it has all the strengths of serial manipulator. Meanwhile, the multi-link aerial robot can trans trought narrow gap, while the normal aerial robot can not achieve.
The extra d.o.f of multi-link aerial robot can also help it do more things. Sugito et al. \cite{sugito_2022_aerial} used multi-link aerial robot to push a door. Shi et al. \cite{shi_2020_aerial} used multi-link aerial robot to grasp a box.
...
Caccavale et al. \cite{caccavale_2015_cooperative} tried to used impedance control to adjust two aerial robots in order to grasp a stick. So we can imitate the system.
