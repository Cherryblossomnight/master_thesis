\chapter{Experiment}
In this chapter, we will demonstrate the effects of the proposed hybrid impedance--admittance control framework. In our experiment, Hydrus is selected as the primary experimental platform to demonstrate the proposed hybrid impedance--admittance control framework, while DRAGON is used  as an extension platform to demonstrate the capability of the proposed framework. The experimental setup, preliminary experiments, parameter adjustment, and experimental results are presented in the following sections.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Experimental Platforms}

\subsection{Hydrus Xi: Primary Experimental Platform}
The underactuated multi-link aerial robot Hydrus Xi used in the experiments is shown in Fig.~\ref{fig:robot_hydrus}. The robot consists of four serial links connected by three rotational joints. All joint axes are parallel, so that the deformation is constrained in a plane. Each link is equipped with a gimbal mudule with a propeller, which enables thrust vectoring through the 1-DoF yaw rotation mechanism. The platform is equipped with an UP-Board single-board computer featuring an Intel Atom x5-Z8350 CPU for onboard computation. 
...
\begin{figure}[!htbp]
    ...
    \caption{The mechanical design of the 3D multi-link aerial robot DRAGON. (a) Overall mechanical structure. (b) End-effector used in surface sliding experiments. (c) 2-DoF Gimbal module. (d) 2-DoF Joints.}
...
Since Hydrus Xi is an underactuated system, the rotational and translational parameters need to be tuned cascadely. Accoding to the principle mentioned in Sec~\ref{sec:imp_param_tuning}, the PD parameters were considered first. To achieve fast convergence to the desired orientation, $\tilde{c}_d$ in roll and pitch axes was set to a relatively large value. Meanwhile, to avoid oscillation, the damping ratio $\zeta$ in these two axes was tuned to approximately $1.2-1.4$. The remaining parameters were then adjusted according to the experimental conditions.
...
\begin{figure}[!htbp]
    ...
    \caption{The multi-link aerial robot was dragged by a rope to simulate the external force on a single direction. The aerial robot moved in order to generate enough consistant force to the external dragging force along the $x$ axis.}
    \label{fig:imp_drag}
\end{figure}
...
As illustrated in Fig.~\ref{fig:imp_drag_result}, during the experiment, Hydrus Xi was gradually dragged away from its reference position until larger position errors became hard to achieve. A larger position error can be observed in the soft case, whereas the rigid case exhibited a smaller position error. Although the same external force was not guaranteed in different cases, the results intuitively showed that a larger $\mu$ leads to a more rigid response, and demonstated that the impedance controller can modify the dynamic performance of the multi-link aerial robot.
