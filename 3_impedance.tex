...
In this chapter, we will introduce the fundamental concept of impedance control and how we can achieve it in multi-link aerial robots, as well as a new insight into impedance control. After that, the special settings for the 2D multi-link aerial robot Hydrus Xi and the 3D multi-link aerial robot DRAGON will be introduced. Finally, since some tasks require the selection of the impedance acting coordinate, we will introduce the task-oriented impedance coordinate selection method.
...
Impedance control models the robot as a second-order system from the perspective of the environment, so that the robot can adjust its own movement according to external forces \cite{hogan_1984_impedance}. To meat different requirements, the dynamic parameters can be freely designed to achieve the desired interaction behavior. For example, by adjusting the stiffness parameters, the robot can be made resilient in certain directions to reject disturbances while remaining compliant in others to maintain safe interaction \cite{bodie_2020_active}. In this section, we will introduce how to obtain the command wrench equation of impedance control.

We will describe the motion in terms of translation and rotation separately. The translational motion is expressed in the world frame $\{W\}$, whereas the rotational motion is expressed in the CoG frame $\{C\}$. Under this convention, the impedance equations are written as:
...
\end{align}
\end{subequations}
where ${\boldsymbol{M}_{d}}\in \mathbb{R}^{3\times3}$ is the desired virtual mass, ${\boldsymbol{I}_{d}}\in \mathbb{R}^{3\times3}$ is the desired virtual inertia, ${\boldsymbol{C}_{td}}\in \mathbb{R}^{3\times3}$, ${\boldsymbol{C}_{rd}}\in \mathbb{R}^{3\times3}$ are the desired linear and rotational damping matrices, and ${\boldsymbol{K}_{td}}\in \mathbb{R}^{3\times3}$, ${\boldsymbol{K}_{rd}}\in \mathbb{R}^{3\times3}$ are the desired linear and rotational stiffness matrices, respectively. All of these matrices are positive definite. These are the dynamics that we want the system to exhibit. Here the error vectors in (\ref{eq:impedance control 1}) are defined as:
...
Next, by substituting the dynamic equations \eqref{eq:dynamics} into the impedance control equations \eqref{eq:impedance control 1} to eliminate $\ddot{\boldsymbol{p}}$ and $\dot{\boldsymbol{\omega}}$, the command wrench applied to the CoG can be derived. The translational component can be written as
...
Finally, for the reason that we will introduce in the next section, we can rewrite the \ref{eq:imp cmd for trans1} and \ref{eq:imp cmd for rot1} as follows. Here we define the normalized linear damping and stiffness as $\tilde{\boldsymbol{C}}_{td} = \boldsymbol{M}^{-1}_d\boldsymbol{C}_{td}$ and $\tilde{\boldsymbol{K}}_{td} = \boldsymbol{M}^{-1}_d\boldsymbol{K}_{td}$, respectively. Using these definitions, \ref{eq:imp cmd for trans1} can be rewritten as
...
\end{equation}
Also, we define the normalized rotational damping and stiffness as $\tilde{\boldsymbol{C}}_{rd} = \boldsymbol{I}^{-1}_d\boldsymbol{C}_{rd}$ and $\tilde{\boldsymbol{K}}_{rd} = \boldsymbol{I}^{-1}_d\boldsymbol{K}_{rd}$, respectively. Equation (\ref{eq:imp cmd for rot1}) can then be rewritten as
...
\end{equation}
These equations help generate the command wrench that we want the rotors to generate. In the next section, we will explain why we rewrite the equations in this form and introduce a new insight into impedance control.
...
\end{equation}
Here, the parameters $m_d$, $c_d$, and $k_d$ are the designed mass, the designed damping, and the designed stiffness, respectively. $\ddot{p}$ is the current acceleration, while $e_v$ and $e_p$ denote the velocity and position errors defined as $e_v = \dot{p}-\dot{p}_{ref}$ and $e_p = p-p_{ref}$, respectively. $f_{ext}$ represents the external force.
...
We define $\tilde{c}_d = \frac{c_d}{m_d}$ and $\tilde{k}_d = \frac{k_d}{m_d}$ as the normalized damping and normalized stiffness, respectively. We also use the mass ratio $\mu$ to replace $\frac{m}{m_d}$, so the equation becomes
\begin{equation}
...
\end{equation}
Note that the errors here are defined as the current values minus the reference values; therefore, we use negative signs in the control law.

We can find that the two equations are very similar. If we choose $\tilde{c}_d=\frac{k_d^{PID}}{m}$ and $\tilde{k}_d=\frac{k_p^{PID}}{m}$, the only differences lie in the integral term $k_i^{PID}\int e_p\mathrm{d}t$ and the external force compensation term $(\mu-1){{f}_{ext}}$. Both of them are designed to reduce errors, but their effects are different in contact situations.

In the context of interaction, due to the contact conditions, there may be a persistent position error between the position reference and the contact boundary in the contact direction, or a persistent external force related to friction in the moving direction. In the case of the contact direction, to reduce the position error, the integral term $k_i^{PID}\int e_p\mathrm{d}t$ will continue to accumulate and increase the command force without bound unless we set a limitation in advance. This means that the integral term is unable to respond well to external forces. On the other hand, the external force term $(\mu-1){{f}_{ext}}$ is related to the estimated external force. It can adjust the final command wrench by considering the external force, while the real contact force varies indirectly with changes in the position error. In the case of the moving direction, the external force term can compensate for part of the friction, which means that it can perform better than PID control that does not have information about the external force. In summary, in contact scenarios, impedance control generally performs better than PID control because it takes external force information into consideration.

On the other hand, impedance control still has many similarities to PID control, so we can get some inspiration from the PID parameter tuning process. Here, we can divide the impedance equation into the PD control part $-m({\frac{c_d}{m_d}e_v+\frac{k_d}{m_d}e_p})$ and the external force part $(\frac{m}{m_d}-1){{f}_{ext}}$. If there is no external force, the external force part disappears, and only the PD control part remains. Therefore, we can use PID tuning methods to determine the parameters $\tilde{c}_d$ and $\tilde{k}_d$, which can now be considered as the corresponding $k_d$ and $k_p$ gains, respectively.

After tuning the parameters in free flight, their effects must also be considered in contact situations. Let us consider the mass ratio parameter $\mu$. If $\mu$ is set to $0$, the compensation term is exactly opposite to the estimated external wrench, so the command will completely compensate for the external force. If $\mu$ is set to $1$, the external force term will disappear, in which case the impedance controller degenerates into a PD controller. If $\mu > 1$, the external force term has the same direction as the external force, so the command will help push the aerial robot in the direction of the external force, which means that the aerial robot will follow the external force. In this situation, the aerial robot can be regarded as relatively compliant, and the stiffness is relatively low. On the other hand, if $\mu < 1$, the external force term has the opposite direction to the external force, which means that the command will help resist the external force. In this situation, the aerial robot will try to reduce its movement in the direction of the external force, and the stiffness is relatively high. In summary, by setting $\mu$ larger or smaller than $1$, the aerial robot can exhibit relatively compliant or rigid behavior. By tuning the parameters depending on the task requirements and environmental conditions, the aerial robot can exhibit different behaviors along different axes.
%
% The impedance control usually set the parameter between $0$ and $\inf$. If the parameter

Based on this discussion, a practical tuning principle can also be obtained. We can first tune $\tilde{c}_d$ and $\tilde{k}_d$ as PD controller gains in free flight, and then design the value of $\mu$ according to the task requirements. Because we use the same set of parameters in both free flight and contact, it is not necessary to switch the control mode before and after contact.

From this perspective, impedance control can be considered as a generalization of pure PD control and feedforward compensation with PD control. We can also consider the equation as a PID controller whose integral term is replaced by an estimated wrench term \cite{bodie_2023_omnidirectional}. If the parameter $\mu$ is set to $1$, the compensation term disappears, and the equation degenerates into pure PD control. By adjusting $\mu$, different interaction behaviors can be achieved to satisfy different task requirements.

In this section, we introduced the fundamental concept of the impedance controller used in multi-link aerial robots. The impedance controller can be considered as an intermediate form between a PD controller and a PD controller with feedforward compensation. The main idea is to adjust the parameter $\mu$ to realize different contact behaviors in order to meet different task requirements. Since the impedance controller considers external force information, it is suitable for tasks with force-interaction requirements, allowing the robot to resist external forces while maintaining safe interaction. Therefore, the characteristics of impedance control are well suited to surface sliding tasks that require appropriate responses to external forces.
...
After understanding the parameter meanings, we can implement impedance control on the multi-link aerial robot. Since current multi-link aerial robots have different mechanical configurations, which could influence their specific control implementations, in this section, we will discuss the realization of impedance control based on the underactuated platform Hydrus and the overactuated platform DRAGON, respectively.




\subsection{Implementation on Hydrus}
To use impedance control on Hydrus, we need to take some special steps. Since Hydru Xi is an underactuated system, the translational and rotational parts must be controlled cascadely.
...
Therefore, the rotor thrusts and gimbal angles required to realize the command wrench are obtained through the above allocation process.
